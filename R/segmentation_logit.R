...
#'   background=list(fundo,ref),sample=2000,fillHull=TRUE,plot=T)
#'
#'   #Segmentacao para separar o objeto de referencia do restante
#'   ref.seg=segmentation_logit(im,foreground=ref,
#'   background=list(fundo,folhas),sample=2000,fillHull=TRUE,plot=T)
#'
...
segmentation_logit=function(im,foreground,background,sample=2000,fillHull=TRUE,TargetPixels="all",plot=F){
  ...
  if(is.list(foreground)){
    ...
    for(i in 1:length(foreground)){
      imm=foreground[[i]]
      if(isFALSE(is.Image(imm))){print("All images used must have 3 bands and be in EBImage format (Todas as imagens utilizadas devem ter 3 bandas e estar no formato do EBImage )")}
      n=ncol(imm@.Data)*ncol(imm@.Data)
      if(sample>n){print("One of the foregrounds images has a smaller number of pixels than the established sample (Uma das imagens tem o numero de pixels inferior ao da amostra estabelecida)")
        ;sample2=n}
  ...
  if(is.list(background)){
    ...
    for(i in 1:length(background)){
      ...
      if(isFALSE(is.Image(imm))){
        print("All images used must have 3 bands and be in EBImage format
              (Todas as imagens utilizadas devem ter 3 bandas e estar no formato do EBImage )")}
      n=ncol(imm@.Data)*ncol(imm@.Data)
  if(sample>n){print("One of the bacgrounds images has a smaller number of pixels than the established sample
  ...
  if(plot==T){print(  display(ImagemSeg))}
  return(ImagemSeg)
