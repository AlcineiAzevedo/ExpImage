#' unçao para a segmentacao de imagens.
#'
...
#' @usage
#'   segmentation(img.band,treshold="otsu",selectHigher=T,
#'   fillHull=FALSE,fillBack=FALSE,TargetPixels="all",plot=F)
...
segmentation=function(img.band,treshold="otsu",selectHigher=T,fillHull=FALSE,
                      fillBack=FALSE,TargetPixels="all", plot=F){
  #library(EBImage)
  if(isFALSE(is.matrix(TargetPixels))){
    b=img.band
    if(treshold=="otsu"){ts=otsu(b);print(
      paste("The treshold by Otsu melhod is (O valor do limiar pelo etodo otsu e:",ts))}
  ...
  if(isTRUE(is.matrix(TargetPixels))){
    ...
    print(paste("The treshold by Otsu melhod is (O valor do limiar pelo etodo otsu e:",ts))}
    if(treshold!="otsu"){ts=treshold}
  ...
  if(plot==T){print(display(MatrizSegentada))}
