#' uncao para obter medidas associadas aos objetos em imagens binarias.
#'
#' @description Esta funcao possibilita a obtenção de medidas associadas aos
...
#'MatrizSegmentada=segmentation(b,treshold = 0.30,fillHull = T,
#'selectHigher = F,plot=T)
#'
#'im2=extract_pixels(im,target =MatrizSegmentada,valueTarget =1,
#'valueSelect = c(0,0,0),plot=T )
#'
#'
...
#'#O limiar pode ser um valor escolhido aleatoriamente (por exemplo: 0.6)
#'MatrizSegmentada2=segmentation(b,treshold = 0.50,fillHull = T,
#'selectHigher = T,plot = T)
#'
...
#'
#'############################################################# #Obtendo a area
#'de folhas de acerola
#'##############################################################
#'
#'
#'#ativar pacote library(EBImage) library(ExpImage) #Abrir imagem
#'im=readImage(example_image(3))
#'
#'
#'#Selecionando o melhor indice para a segmentacao r=gray_scale(im,method =
#'"r",plot=T) g=gray_scale(im,method = "g",plot=T) b=gray_scale(im,method =
#'"b",plot=T)
#'
#'
#'#A banda de azul foi a que melhor discriminou #O limiar pode ser um valor
#'#escolhido aleatoriamente (por exemplo: 0.6)
#'MatrizSegmentada=segmentation(b,treshold = 0.6,fillHull = F,selectHigher =
#'F,plot=T)
#'
#'#O limiar tambem pode ser estabelecido pelo metodo de otsu
#'MatrizSegmentada2=segmentation(b,treshold = "otsu",fillHull = T,
#'selectHigher =F,plot=T)
#'
#'#Obter medidas de cada objeto medidas=measure_image(MatrizSegmentada2)
...
#'medidas$ObjectNumber Estimativas=medidas$measures
#'
#'#Plotar resultados das areas em pixel e salvar em um arquivo chamado
#'"teste.jpg"
...
#'
#'plot_meansures(im,medidas$measures[,1],coordy=medidas$measures[,2],
#'text=round(medidas$measures[,11],2),cex = 0.9,pathSave ="teste.jpg",col="blue" ,plot=T)
#'##############################################################################
...
#'
#'plot_meansures(im,medidas$measures[,1],coordy=medidas$measures[,2],
#'text=rownames(medidas$measures),cex= 0.9,pathSave ="teste.jpg",col="blue",plot=T )
...
#'text=round(medidas3$measures[,3],2),cex = 0.9,col="blue",plot=T)
#'}
